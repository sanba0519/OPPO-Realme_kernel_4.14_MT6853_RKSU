...
static bool dedup_should_print(uid_t uid, u8 type, const char *content, size_t len)
{
    ...
    if (e->key.crc == key.crc && e->key.uid == key.uid && e->key.type == key.type && (now - e->ts_ns) < delta_ns) {
...
void ksu_sulog_report_su_grant(uid_t uid, const char *comm, const char *method)
{
    ...
    snprintf(log_buf, sizeof(log_buf), "[%s] SU_GRANT: UID=%d COMM=%s METHOD=%s PID=%d\n", timestamp, uid, full_comm,
...
void ksu_sulog_report_su_attempt(uid_t uid, const char *comm, const char *target_path, bool success)
{
    ...
    snprintf(log_buf, sizeof(log_buf), "[%s] SU_EXEC: UID=%d COMM=%s TARGET=%s RESULT=%s PID=%d\n", timestamp, uid,
             full_comm, target_path ? target_path : "unknown", success ? "SUCCESS" : "DENIED", current->pid);
...
void ksu_sulog_report_permission_check(uid_t uid, const char *comm, bool allowed)
{
    ...
    snprintf(log_buf, sizeof(log_buf), "[%s] PERM_CHECK: UID=%d COMM=%s RESULT=%s PID=%d\n", timestamp, uid, full_comm,
...
void ksu_sulog_report_manager_operation(const char *operation, uid_t manager_uid, uid_t target_uid)
{
    ...
    snprintf(log_buf, sizeof(log_buf), "[%s] MANAGER_OP: OP=%s MANAGER_UID=%d TARGET_UID=%d COMM=%s PID=%d\n",
...
void ksu_sulog_report_syscall(uid_t uid, const char *comm, const char *syscall, const char *args)
{
    ...
    snprintf(log_buf, sizeof(log_buf), "[%s] SYSCALL: UID=%d COMM=%s SYSCALL=%s ARGS=%s PID=%d\n", timestamp, uid,
             full_comm, syscall ? syscall : "unknown", args ? args : "none", current->pid);
